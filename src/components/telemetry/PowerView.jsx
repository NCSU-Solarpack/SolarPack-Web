  ...
  return (
    <div>
      ...
      <div className="tlm-tiles cols-4">
        <StatTile big label="Battery power" value={power(battW)} accent={battW < 0 ? 'var(--tlm-good)' : 'var(--tlm-muted)'} icon={Battery}
          sub={`${num(pkt.batteryVoltage, 1)} V · ${num(pkt.currentDraw, 1)} A`} />
        <StatTile big label="Solar input" value={power(solarW)} accent={pkt.solarCharging ? 'var(--tlm-good)' : 'var(--tlm-muted)'} icon={Sun}
          sub={`${num(pkt.solarVolts || pkt.solarVoltage, 1)} V · ${num(pkt.solarAmps, 1)} A`} />
        <StatTile big label="Motor power" value={power(motorW)} accent="var(--tlm-muted)" icon={Cpu}
          sub={`${num(busV, 0)} V bus · ${num(busA, 0)} A`} />
        <StatTile big label="Net draw" value={power(netW)} accent={netW < 0 ? 'var(--tlm-good)' : 'var(--tlm-muted)'} icon={Zap}
          ...
          <div className="tlm-chart">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={series} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="motorW" name="Motor" stroke="#5b8def" dot={false} strokeWidth={2} isAnimationActive={false} />
                <Line type="monotone" dataKey="solarW" name="Solar" stroke="#d99a3d" dot={false} strokeWidth={2} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="tlm-legend">
            <span><i style={{ background: '#e31b23' }} />Battery</span>
            <span><i style={{ background: '#5b8def' }} />Motor</span>
            <span><i style={{ background: '#d99a3d' }} />Solar</span>
          </div>
        </Card>
          ...
          <div className="tlm-chart">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={series} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="soc" stroke="#2ecc71" fill="url(#socfill)" strokeWidth={2} isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <div className="tlm-tiles cols-4">
        ...
        <StatTile label="Est. range" value={num(pkt.batteryRange, 0)} unit="mi" accent="var(--tlm-muted)" icon={Route} />
        <StatTile label="Solar volts" value={num(pkt.solarVolts || pkt.solarVoltage, 1)} unit="V" accent="var(--tlm-muted)" icon={Sun} />
        <StatTile label="Solar amps" value={num(pkt.solarAmps, 1)} unit="A" accent="var(--tlm-muted)" icon={Sun} />
